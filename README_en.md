...
- **Python 3.8+**
- **Libraries: `requests`, `tqdm>=4.67.3`, `dirhash==0.5.0`, `typer==0.24.2`**
...
|---------|-------------|
| **Common** |
| `ldu-tool -V/--version`| Show tool version|
| `ldu-tool -h/--help` / `ldu-tool install -h/--help` | Display command help |
| **Install** |
| `ldu-tool install` / `ldu-tool install <game_name>` | Starts basic installation in the directory where the script is located. Downloads DLC and the modified `libsteam_api.so` library |
...
| **List** ||
|`ldu-tool list`|Displays available games for unlocking|
...
4. **Download Content:** Downloads ZIP archives with DLC files from an external file server.
...
- **Mods not working:** Currently, the solution to this issue can be found in **[this issue](https://github.com/IAMVanilka/linux-dlc-unlocker-tool/issues/3)**.
